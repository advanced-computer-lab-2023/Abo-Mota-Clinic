...
function Package({ id, name, discounts, cost, onEdit, onDelete }) {
  ...
  const renderedDiscounts = discounts.map((discount, index) => (
  ...
  const handleDiscountChange = (index, event) => {
    ...
    updatedDiscounts[index] = { ...updatedDiscounts[index], content: event.target.value };
    ...
    content = (
      <div>
        <form onSubmit={handleSubmit}>
          ...
          <hr className="card-hr" />
          {updatedPackage.discounts.map((discount, index) => (
            <div key={index}>
